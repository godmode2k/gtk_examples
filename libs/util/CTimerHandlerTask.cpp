...
Filename:	CTimerHandlerTask.cpp
...
// ---------------------------------------------------------------
// Ctor
CTimerHandlerTask::CTimerHandlerTask(void) : TAG("CTimerHandlerTask") {
	__LOGT__( TAG, "CTimerHandlerTask()" );

	__init();
}

// Dtor
CTimerHandlerTask::~CTimerHandlerTask() {
	__LOGT__( TAG, "~CTimerHandlerTask()" );
...
// Initialize
void CTimerHandlerTask::__init(void) {
	__LOGT__( TAG, "__init()" );
	...
	m_msg = NULL;
#endif
	m_delay = e_timerHandlerTaskDelay_NONE;
	m_working = false;
}

// Release
void CTimerHandlerTask::__release(void) {
...
void CTimerHandlerTask::updates(void) {
	//__LOGT__( TAG, "updates()" );
}

void CTimerHandlerTask::wait_for_thread_finished(void) {
...
void CTimerHandlerTask::set_delay(e_TimerHandlerTaskDelay_t delay) {
	//__LOGT__( TAG, "set_delay()" );

	m_delay = (delay > e_timerHandlerTaskDelay_LONG)? e_timerHandlerTaskDelay_SHORT : delay;
}

bool CTimerHandlerTask::run(void* user_data) {
	//__LOGT__( TAG, "run()" );
	
	if ( get_working() ) {
		//__LOGT__( TAG, "run(): working... SKIP" );
		return false;
	}
	...
	__LOGT__( TAG, "run(): Message = %s, delay = %d(ms)", get_message(), get_delay() );
	*/
	...
	if ( !this->execute(NULL, "%s", (char*)user_data) ) {
		__LOGT__( TAG, "run(): execute [FAIL]" );

		destroy_ui();

		set_working( false );
		set_delay( e_timerHandlerTaskDelay_NONE );
...
#if 0
bool CTimerHandlerTask::show(GtkWindow* pWindow, gpointer user_data,
...
void CTimerHandlerTask::task_in_thread(void* user_data) {
...
void CTimerHandlerTask::destroy_ui(void) {
...
void CTimerHandlerTask::stop(void) {
	__LOGT__( TAG, "stop()" );

	set_working( false );
}

void CTimerHandlerTask::__stop(void) {
	__LOGT__( TAG, "__stop()" );
...
void* CTimerHandlerTask::inBackground(std::vector<void*>* pvecVal) {
	...
	do {
		...
		// perform the task
		{
			...
			if ( pvecVal && (pvecVal->size() > 0) ) {
				...
				//	__LOGT__( TAG, "inBackground(): str -> %s", (char*)(*iter) );
				//}
				//__LOGT__( TAG, "inBackground(): str[0] -> %p", (void*)(*pvecVal)[0] );

				user_data = (void*)(*pvecVal)[0];
				__LOGT__( TAG, "inBackground(): str[0] -> %p", user_data );
			}
...
void CTimerHandlerTask::progressUpdate(void* pVal) {
